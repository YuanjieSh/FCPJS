...
def standard_conformal(cal_scores_all, cal_labels, val_scores_all, val_labels, alpha, exact_coverage=False):
    ...
    class_coverage_metrics, coverage_metrics, set_size_metrics = compute_all_metrics(val_labels, standard_preds, alpha)
    
    return standard_qhat, class_coverage_metrics, coverage_metrics, set_size_metrics
...
def classwise_conformal(totalcal_scores_all, totalcal_labels, val_scores_all, val_labels, alpha, class_gap, 
    ...
    # print(f'Class Conditional Quantile:{classwise_qhats}')
    ...
    return classwise_qhats, class_coverage_metrics, coverage_metrics, set_size_metrics
...
#                                                            split=split, # Balanced or stratified sampling 
    ...
    if (val_scores_all is not None) and (val_labels is not None):
        ...
        class_coverage_metrics, class_cov_metrics, set_size_metrics = compute_all_metrics(val_labels, preds, alpha,
        ...
        return qhats, preds, class_coverage_metrics, class_cov_metrics, set_size_metrics
    else:
        return qhats
...
def get_APS_scores(softmax_scores, labels, randomize=True, seed=0):
    ...
    sorted_probs, sorted_indices = softmax_scores.sort(dim=1, descending=True)

    # Compute cumulative sums along sorted class probs
    cumsum_probs = sorted_probs.cumsum(dim=1)

    # Undo the sort to get cumulative probs at the original class label positions
    inverse_indices = sorted_indices.argsort(dim=1)
    scores = cumsum_probs.gather(1, inverse_indices)[torch.arange(n), labels]
    ...
    if not randomize:
        ...
        U = torch.rand(n, device=softmax_scores.device)
...
def get_APS_scores_all(softmax_scores, randomize=True, seed=0):
    ...
    # Reorder cumulative sums back to original class index order
    inverse_indices = sorted_indices.argsort(dim=1)
    scores = cumsum_probs.gather(1, inverse_indices)
    ...
    if not randomize:
        ...
        np.random.seed(seed)
        U = torch.rand(n, num_classes, device=softmax_scores.device)
...
def get_RAPS_scores(softmax_scores, labels, lmbda=.01, kreg=5, randomize=True, seed=0):
    ...
    '''
    n = len(labels)
    sorted, pi = softmax_scores.sort(dim=1, descending=True) # pi is the indices in the original array
    ...
    if not randomize:
        ...
        U = torch.rand(n, device=softmax_scores.device)
        # U = np.random.rand(n) # Generate U's ~ Unif([0,1])
        randomized_scores = scores - U * softmax_scores[torch.range(n), labels]
        return randomized_scores
...
def compute_all_metrics(val_labels, preds, alpha, cluster_assignments=None):
    ...
    class_cov_metrics = {
                         # 'mean_class_cov_gap': avg_class_cov_gap, 
                         # 'undercov ratio': undercov_ratio, 
                         # 'Class Conditional Coverage std': cc_var, 
                         'average set size': np.mean(curr_set_sizes),
                         'marginal_cov': marginal_cov,
                         #'class-conditional coverage': class_cond_cov
    ...
    # print('Class Conditional Coverage variance:', cc_std)
    ...
    return class_coverage_metrics, class_cov_metrics, set_size_metrics
